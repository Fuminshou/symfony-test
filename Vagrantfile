...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.synced_folder "./", "/var/www/symfony", type: "nfs", mount_options: ['rw', 'vers=3', 'tcp', 'fsc']
  ...
  config.vm.provision :shell, :path => "vagrant/scripts/install-ansible.sh", :args => "/var/www/symfony/vagrant"
  config.vm.provision :shell, :path => "vagrant/scripts/run-ansible.sh", :args => "/var/www/symfony/vagrant"
